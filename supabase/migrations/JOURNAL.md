...
| `202609021000_pret_grimpeur` | Table `pret` (rencontre, grimpeur, club_accueil) = **prêt persistant** distinct de l'équipe : écriture admin (R35), lecture coach d'accueil. `grimpeur_select` élargi (`voit_grimpeur_via_pret` → roster) + `composition_insert` élargi (`peut_engager_prete` → le coach affecte/ré-affecte un prêté, R36). Chemin admin direct conservé. Spec #1 R35/R36, spec #5 R12/R13 (rév. 2026-09-02) | _à appliquer_ | _reporté_ |
