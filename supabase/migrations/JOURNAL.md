...
| `202609020900_rls_grimpeur_prete_lisible` | Grimpeur PRÊTÉ visible/gérable par le coach d'accueil : `grimpeur_select` élargi (helper `voit_grimpeur_via_engagement` — nom d'un grimpeur composé dans une équipe que je coache, R13) + `composition_delete` sur `peut_retirer_composition` (équipe d'accueil + gating de phase — retrait d'un prêté, R36). INSERT cross-club inchangé (admin, R35). Corrige « (inconnu) » + retrait bloqué (spec #5) | _à appliquer_ | _reporté_ |
